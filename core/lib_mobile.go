...
func handleStartTun(fd int, callback unsafe.Pointer) bool {
	...
	if fd != 0 && currentConfig == nil {
		tunError = "startTUN called before setupConfig"
		log.Warnln("[APP] %s", tunError)
		return false
	}
	tunLock.Lock()
	defer tunLock.Unlock()
	...
	if fd != 0 {
		...
		initTunHook()
		tunListener, err := t.Start(fd, currentConfig.General.Tun.Device, currentConfig.General.Tun.Stack, currentConfig.General.Tun.DisableICMPForwarding, uint32(currentConfig.General.Tun.MTU), currentConfig.General.IPv6)
		if tunListener != nil {
			log.Infoln("TUN address: %v", tunListener.Address())
			handler.listener = tunListener
		} else {
			tunError = fmt.Sprintf("%v", err)
			log.Errorln("[APP] TUN startup failed: %v", err)
			fmt.Printf("[BETTBOX_TUN] startup failed: %v\n", err)
			removeTunHook()
...
//export getTunError
func getTunError() *C.char {
	return C.CString(tunError)
}
